...
def cleanup():
    ...
    print "Initialize.py Cleanup Operations Complete"
...
def final_cleanup():
    ...
    print "myCoGe Complete"


def initialize():
    # Check for temp files, make if absent
    if not path.exists('./temp'):
        mkdir('./temp')
    if not path.exists('./temp/pickles'):
        mkdir('./temp/pickles')

    # Check for data files, make if absent
    if not path.exists('./data'):
        mkdir('./data')
    if not path.exists('./data/vcfs'):
        mkdir('./data/vcfs')
...
def isync(password):
    ...
    """
    initiate = spawn('iinit')
    initiate.expect("Enter your current iRODS password:")
    initiate.sendline(password)

    spawn('irsync -r ./data/vcfs i:coge_data/mycoge')
...
def send_email_log(new_files_added_list, attachments):
    ...
    email_from = account
    email_to = ['ahaug@email.arizona.edu']
    email_subject = "myCoGe Log - %s" % date
    email_text = "myCoGe was successfully executed, with %s new experiments added today.\n" \
